...
const X86_PCLMULQDQ: u8 = 1 << 0;
const X86_AVX2: u8 = 1 << 1;
const X86_VPCLMULQDQ: u8 = 1 << 2;
const AARCH64_NEON: u8 = 1 << 3;
const AARCH64_PMULL: u8 = 1 << 4;
...
impl CpuCapabilities {
    ...
    #[cfg(test)]
    pub(crate) const fn from_test_parts(architecture: Architecture, features: u8) -> Self {
        Self {
            architecture,
            features: features & 0x1f,
        }
    }
}
...
mod tests {
    ...
    fn backend_requirements_include_architecture_and_prerequisites() {
        let all_x86 = CpuCapabilities::from_test_parts(
            Architecture::X86_64,
            X86_PCLMULQDQ | X86_AVX2 | X86_VPCLMULQDQ,
        );
