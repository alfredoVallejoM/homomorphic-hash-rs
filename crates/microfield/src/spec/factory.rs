...
use super::{
    ...
    model::{
        FieldManifest, GeneratedArtifacts, IsaProfileSchedule, PortableOptimizationPlan,
        PortableReductionStrategy, ValidatedFieldSpec, VerifiedIsaProfile,
    },
};
...
impl BinaryFieldFactory {
    ...
    pub fn generate(&self) -> Result<GeneratedFieldPackage, BinaryFieldFactoryError> {
        ...
        Ok(GeneratedFieldPackage {
            ...
            verified_isa_profile: plan.verified_isa_profile().clone(),
            artifacts,
        })
    }
}
...
impl GeneratedFieldPackage {
    ...
    pub const fn codegen_abi_version(&self) -> u32 {
        crate::__private::CURRENT_CODEGEN_ABI_VERSION
    }
    ...
    /// Returns the verified, target-neutral ISA compatibility profile.
    #[must_use]
...
fn render_rust_module(
    ...
    let isa_schedule = match plan.verified_isa_profile().schedule() {
        IsaProfileSchedule::Fixed => "::microfield::ScheduleKind::Fixed",
        IsaProfileSchedule::DataDependent => "::microfield::ScheduleKind::DataDependent",
    };
    let (multiply_body, square_body, reduce_body, dense_tail_constant) = match plan
        ...
        .reduction()
    {
        PortableReductionStrategy::LowTailFold => (
            ...
            format!(
                "::microfield::__private::reduce_low_tail::<{limbs}, {}, {low_tail}>(wide)",
                limbs * 2
            ),
            String::new(),
        ),
        PortableReductionStrategy::SparseTermFold => (
            ...
            format!(
                "::microfield::__private::reduce_sparse::<{limbs}, {}>(wide, DEGREE, MODULUS_EXPONENTS_DESC)",
                limbs * 2
            ),
            String::new(),
        ),
        PortableReductionStrategy::DenseWordFold => (
            ...
            format!(
                "::microfield::__private::reduce_dense::<{limbs}, {}>(wide, DEGREE, &MODULUS_TAIL_WORDS)",
                limbs * 2
            ),
    ...
    for (token, value) in [
        (
            "__CODEGEN_ABI_VERSION__",
            crate::__private::CURRENT_CODEGEN_ABI_VERSION.to_string(),
        ),
        ...
        ("__REDUCE_BODY__", reduce_body),
        (
            "__ISA_PROFILE_DIGEST__",
            format!("{:?}", plan.verified_isa_profile().profile_digest()),
        ),
        ("__ISA_SCHEDULE__", isa_schedule.to_owned()),
        (
            "__FIELD_ID__",
...
const _: () = assert!(::microfield::__private::supports_codegen_abi(__CODEGEN_ABI_VERSION__));
...
const VERIFIED_ISA_PROFILE_DIGEST: &str = __ISA_PROFILE_DIGEST__;
...
static VERIFIED_ISA_STRATEGY: ::microfield::__private::VerifiedIsaStrategy<
    __TYPE__, __LIMBS__, __WIDE_LIMBS__
> = ::microfield::__private::VerifiedIsaStrategy::new();
...
impl ::microfield::__private::PortableField for __TYPE__ {
    ...
    fn __kernel_catalog() -> ::microfield::KernelCatalog<Self> {
        VERIFIED_ISA_STRATEGY.__kernel_catalog(&PORTABLE_STRATEGY)
    }
}

impl ::microfield::__private::VerifiedBinaryIsaField<__LIMBS__, __WIDE_LIMBS__> for __TYPE__ {
    const PROFILE_DIGEST: &'static str = VERIFIED_ISA_PROFILE_DIGEST;
    const SCHEDULE: ::microfield::ScheduleKind = __ISA_SCHEDULE__;

    fn __into_limbs(self) -> [u64; __LIMBS__] { self.0 }

    fn __from_reduced_limbs(limbs: [u64; __LIMBS__]) -> Self { Self(limbs) }

    fn __reduce_wide(wide: [u64; __WIDE_LIMBS__]) -> [u64; __LIMBS__] {
        __REDUCE_BODY__
    }
}
